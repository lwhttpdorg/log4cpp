...
console_output *console_output_config::instance = nullptr;
log_lock console_output_config::instance_lock;

console_output *console_output_config::get_instance(const console_output_config &self)
{
    if (instance == nullptr)
    {
        instance_lock.lock();
        if (instance == nullptr)
        {
            instance = console_output::builder::new_builder().set_out_stream(self.out_stream).build();
        }
