...
namespace log4cpp {
	...
	const char *UDP_OUTPUT_GOODBYE = "goodbye";

	std::atomic_bool udp_output::running{true};

	int create_udp_socket(const net::sock_addr &saddr) {
		int fd;
	...
	void accept_worker(int listen_fd, std::unordered_set<net::sock_addr> &clients) {
		fd_set read_fds;
		FD_ZERO(&read_fds);
		...
		while (udp_output::running) {
			...
			if (FD_ISSET(listen_fd, &tmp_fds)) {
				...
				size_t len = recvfrom(listen_fd, buffer, sizeof(buffer) - 1, 0,
				...
				if (client_addr.ss_family == AF_INET) {
					...
					struct sockaddr_in6 *client_addr_in6 = reinterpret_cast<struct sockaddr_in6 *>(&client_addr);
	...
	std::shared_ptr<udp_output> udp_output::builder::build() {
		...
		int server_fd = create_udp_socket(saddr);
		if (server_fd == -1) {
	...
	udp_output::builder udp_output::builder::new_builder() {
		udp_output::builder builder = udp_output::builder{};
	...
	void udp_output::log(log_level level, const char *fmt, va_list args) {
		...
		for (auto &client: this->clients) {
			if (client.addr.family == net::net_family::NET_IPv4) {
				...
				(void) sendto(this->fd, buffer, used_len, 0, reinterpret_cast<struct sockaddr *>(&client_addr),
				              sizeof(client_addr));
			}
			else {
