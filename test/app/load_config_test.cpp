...
void tcp_appender_cfg_check(const nlohmann::json &tcp_appender, const log4cpp::tcp_appender_config *cfg) {
	const log4cpp::common::net_addr actual_addr = cfg->get_local_addr();
...
void appenders_cfg_check(const nlohmann::json &appenders_json, const log4cpp::config::log_appender &appenders_cfg) {
...
void configuration_cfg_check(const nlohmann::json &expected_json, const log4cpp::config::log4cpp *config) {
	// Logger pattern
	const std::string &logger_pattern = config->log_pattern;
	logger_pattern_cfg_check(expected_json, logger_pattern);
	// Appenders
	const log4cpp::config::log_appender &appender_cfg = config->appenders;
